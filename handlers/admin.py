...
@router_admin.inline_query(F.query.startswith('$'))
async def find_users(query:InlineQuery):
    data=await admin_info.search_users(query)
    kb=InlineKeyboardBuilder()
    kb.add(InlineKeyboardButton(text='edit', switch_inline_query_current_chat='edit'))
    results=[InlineQueryResultArticle(description='Full information',title=f"{i[0]}",id=i[0],input_message_content=InputTextMessageContent(message_text=f"<b>Full name - <i>{i[0].upper()}</i>\nPhone number - <i>{i[1]}</i>\nActive cources - <i>{i[2]}</i>\nRole - <i>{i[4]}</i>\nExtra role - <i>{i[5]}</i>\nMonthly payment - <i>{i[6]}</i>\nInvite people - <i>{i[7]}</i>\nCashback - <i>{i[8]}</i>\nDebt - <i>{i[9]}</i>  </b>",parse_mode=ParseMode.HTML),reply_markup=kb.as_markup())for i in data]
    await query.answer(results=results,is_personal=True)

@router_admin.callback_query(F.data=='edit')
async def edit_users(callback:CallbackQuery):
    await callback.edit_text("Choose the group")

@router_admin.chosen_inline_result(F.query=='edit')
...
#@router_admin.callback_query(F.data=='infousers')
